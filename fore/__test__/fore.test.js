...
describe("Foresight Test", () => {
	...
	beforeEach(() => {
		...
		foresight = new Foresight({
			...
			apiUrl: mockApiUrl,
			maxEntriesBeforeAutoFlush: mockMaxEntriesBeforeAutoFlush,
		});
	...
	describe("createSimpleEvalset", () => {
		it("should create a simple evalset", async () => {
			...
			const response = await foresight.createSimpleEvalset({
				evalsetId: mockEvalsetId,
				queries: mockQueries,
			});
	...
	describe("createSimpleEvalsetWithReferences", () => {
		it("should create a simple evalset with references", async () => {
			...
			const response = await foresight.createSimpleEvalset({
				evalsetId: mockEvalsetId,
				queries: mockQueries,
				referenceAnswers: mockReferenceAnswers,
			});
	...
	describe("createSimpleEvalsetWithNotEnoughReferences", () => {
		it("should create a simple evalset with not enough references", async () => {
			const response = foresight.createSimpleEvalset({
				evalsetId: mockEvalsetId,
				queries: mockQueries,
				referenceAnswers: mockReferenceAnswers.slice(0, 1),
			});

			await expect(response).rejects.toThrowError(
				"Number of queries and references must match."
			);
		});
	});
	...
	describe("getEvalset", () => {
		it("should get the evaluation set with metadata", async () => {
			...
			const response = await foresight.getEvalset({
				evalsetId: mockEvalsetId,
			});
	...
	describe("getEvalrunQueries", () => {
		it("should get the queries associated with an eval run", async () => {
			...
			const response = await foresight.getEvalrunQueries({
				experimentId: mockExperimentId,
			});
	...
	describe("createEvalrun", () => {
		it("should create an evaluation run", async () => {
			...
			const response = await foresight.createEvalrun({
				runConfig: {
					evalsetId: mockEvalsetId,
					experimentId: mockExperimentId,
					metrics: mockMetrics,
	...
	describe("getEvalrunDetails", () => {
		it("should get evaluation run details", async () => {
			...
			const response = await foresight.getEvalrunDetails({
				experimentId: mockExperimentId,
				convertToDataframe: false,
			});
